...
function customerList(){
    ...
    foreach($result as $clientInfo){
        $result2 =$query->customerNotification($clientInfo['id']);
        foreach($result2 as $clientInfo2){
            ...
            if((dueDateFigure($dueDate, $now) < 5) && ($clientInfo2['notification'] == 0)){
                ...
                ?>
                    <tr>
                        ...
                        <td>$daysLeft</td>
                        <td>
                        <form id='notification' name='notification' action='notificationaction.php' method='POST'>
                            <input type='hidden' id='id' name='id' value='{$clientInfo['id']}'>
                            <input type='submit' class='btn btn-success' id = 'submit{$clientInfo['id']}' 
                            name='submit{$clientInfo['id']}' value = 'Notify {$clientInfo['customer_id']}'>
                        </form>
                        </td>
                    </tr>   
                ";
                ...
                if((dueDateFigure($dueDate, $now) < 5) && ($clientInfo2['notification'] == 0)){
                    ?>
                        <tr>
                            ...
                            <td>$daysLeft</td>
                            <td>
                            <form role='form' action='notificationaction.php' method='POST' id='notification' name='notification'>
                                <input type='hidden' id='id' name='id' value='{$clientInfo['id']}'>
                                <input type='submit' class='btn btn-success notification' id = 'submit{$clientInfo['id']}' 
                                name='submit{$clientInfo['id']}' value = 'Notify {$clientInfo['customer_id']}'>
                            </form>
                            </td>
                        </tr>   
